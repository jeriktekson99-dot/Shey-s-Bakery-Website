...
export const EditProductModal: React.FC<EditProductModalProps> = ({
  ...
  const [category, setCategory] = useState<string>('Pastries');
  ...
  useEffect(() => {
    if (product) {
      ...
      setBox10(product.boxVariants?.includes('Box of 10') || false);
      setBox15(product.boxVariants?.includes('Box of 15') || false);
      setBox20(product.boxVariants?.includes('Box of 20') || false);
  ...
  const handleExecuteSave = () => {
    ...
    const primaryImage = images.length > 0 ? images[0] : product.image;
    const parsedPrice = parseFloat(String(basePrice));
    const finalBasePrice = isNaN(parsedPrice) || parsedPrice < 0 ? product.basePrice : parsedPrice;
    ...
    const updated: AdminProduct = {
      ...
      boxVariants,
      leadTime: product.leadTime || '24 hrs',
      inStock,
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 z-50 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3.5 py-2.5 rounded-xl border border-stone-300 text-sm font-medium focus:outline-none focus:border-[#d01617] focus:ring-1 focus:ring-[#d01617] bg-white text-[#4a170a]"
              />
            </div>
      ...
      {isConfirmOpen && (
        <AdminConfirmationModal
          ...
          portalName="DATABASE MODIFICATION PORTAL"
          title="CONFIRM PRODUCT EDITS"
          message={
            <p>
              Are you sure you want to save modifications to record <strong className="font-mono font-bold text-[#4a170a]">{product.name}</strong> ({product.id})? This will update the live catalog, price, and packaging variants immediately.
            </p>
          }
          cancelText="CANCEL"
